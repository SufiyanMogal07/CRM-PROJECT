...
function addUsersFNC($conn,$data) {
    $name = $data['name'];
    $phone = $data['phone'];
    $email = $data['email'];
    ...
    $passport = $data['passport'];
     // Validate input data
     if (empty($name) || empty($phone) || empty($email) || empty($address) || empty($city) || empty($passport)) {
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        ...
        $stmt = $conn->prepare("INSERT INTO users (name,phone,email,address,city,passportno) VALUES(?,?,?,?,?,?)");    
        $stmt->bind_param("ssssss",$name,$phone,$email,$address,$city,$passport);
